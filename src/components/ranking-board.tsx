...
// steamAppId → data.ts 게임 정보 매핑
function buildEntry(steamAppId: number, rank: number, country: string): RankEntry {
  const game = games.find((g) => g.steamAppId === steamAppId);
  return {
    rank,
    steamAppId,
    title: game?.title ?? `Game #${steamAppId}`,
    scoreText: game ? `종합 점수 ${game.score}.0` : '-',
    badge: game?.discountRate ? `${game.discountRate}% 할인 중` : '할인 없음',
    priceKr: game?.prices.kr ?? '-',
    priceJp: game?.prices.jp ?? '-',
    discountRate: game?.discountRate ?? 0,
  };
}
...
export function RankingBoard() {
  ...
  const fetchRanking = useCallback(async (tab: TabKey) => {
    ...
    try {
      ...
      if (json.status === 'success' && Array.isArray(json.data)) {
        const entries: RankEntry[] = json.data.map((id: number, idx: number) =>
          buildEntry(id, idx + 1, country)
        );
  ...
  // 탭 변경 시 API 호출
  useEffect(() => {
    if (TAB_TO_COUNTRY[active]) {
      fetchRanking(active);
    }
  }, [active, fetchRanking]);

  // ── best/worst (더미 — DB 연결 후 교체) ──────────────────
  const bestEntries = useMemo(() =>
    [...games]
      .sort((a, b) => b.score - a.score)
      .slice(0, 10)
      .map((g, idx) => ({
        rank: idx + 1, steamAppId: g.steamAppId, title: g.title,
        ...
        <>
          ...
          {isApiTab && apiError && (
            <div className="mb-4 rounded-xl border border-red-400/20 bg-red-400/8 px-4 py-3 text-sm text-red-300">
              ⚠️ 랭킹 데이터를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.
            ...
            <div className="space-y-3">
              {visible.map((entry) => (
                ...
                >
                  ...
                  <img
                    src={getSteamHeader(entry.steamAppId)}
                    alt={entry.title}
